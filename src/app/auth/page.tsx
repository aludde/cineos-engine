  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // 2. THE PATCH: Immediately create their public Profile
        if (authData.user) {
    ...
    <main className="min-h-screen bg-black text-white flex flex-col relative">
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative z-10">
        <div className="w-full max-w-md bg-neutral-950 border border-neutral-900 p-8 shadow-2xl">
          <h1 className="text-3xl font-extrabold tracking-tighter uppercase mb-6">{isLogin ? 'System Access' : 'Initialize Profile'}</h1>
