...
export default function AuthPortal() {
  ...
  // This catches successful logins from BOTH Email AND Google
  useEffect(() => {
  ...
  const handleMigration = async (user: any) => {
    // Failsafe: Ensure their public profile exists (especially for new Google users)
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          // Send them right back here after Google authenticates them so the listener fires
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Notice there is no router.push() here. The Listener above handles it automatically!
    } catch (error: any) {
      alert(error.message);
      setLoading(false);
    }
    ...
    <main className="min-h-screen bg-black text-white flex flex-col relative">
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative z-10">
        <div className="w-full max-w-md bg-neutral-950 border border-neutral-900 p-8 shadow-2xl">
          <h1 className="text-3xl font-extrabold tracking-tighter uppercase mb-6">{isLogin ? 'System Access' : 'Create New Account'}</h1>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="w-full bg-[#E62B1E] hover:bg-white hover:text-black text-white py-4 mt-4 font-bold uppercase tracking-widest text-sm transition-all flex justify-center items-center">
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (isLogin ? 'Engage' : 'Create')}
            </button>
          </form>
          ...
          <button onClick={() => setIsLogin(!isLogin)} className="w-full mt-6 text-xs text-neutral-500 uppercase tracking-widest hover:text-white transition-colors">
            {isLogin ? 'Need a profile? Create a new account.' : 'Already a user? Log in.'}
          </button>
        </div>
      </div>
    </main>
